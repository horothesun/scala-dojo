...
object StackSpec extends Properties("Stack") {
  ...
  property("n pushes followed by n pops leave any Stack unchanged") =
    forAll (elementListGen, stackGen) { (ts, stack) =>
      val stackAfterPushes = ts.foldLeft(stack)((s, t) => push(t)(s))
      val stackAfterPushesAndPops = ts.foldLeft(stackAfterPushes)((s, _) =>
  ...
  property("pushing all elements of a list in a stack then popping them out produces the same list") =
    forAll (elementListGen, stackGen) { (ts, stack) =>
      val stackAfterPushes = ts.foldLeft(stack)((s, t) => push(t)(s))
      val poppedList = ts.foldLeft((List[Int](), stackAfterPushes))((res, _) =>
        pop(res._2) match {
  ...
  property("fromList and toList should produce same list") =
    forAll { s : List[Int] => stackFromList(s).toList == s }

  property("pushBatch of one element is the same as push") =
    forAll (elementGen, stackGen) { (t, stack) =>
      pushBatch(List(t))(stack) == push(t)(stack)
    }

  property("popBatch once is the same as pop") =
    forAll (elementGen, stackGen) { (t, stack) =>
      pushBatch(List(t))(stack) == push(t)(stack)
    }
