...
object TextStatistics {
  ...
  def sentencesFromString: String => List[Sentence] = { s =>
    s.split('.').toList.filterNot(_.isEmpty)
  ...
  def wordCountSentence: Sentence => Int = {
    case Sentence(ws) => ws.length
  }
  ...
  def longestWordInSentence: Sentence => Int = {
    case Sentence(ws) => ws.map {
      case Word(s) => s.length
    }.max
  }
